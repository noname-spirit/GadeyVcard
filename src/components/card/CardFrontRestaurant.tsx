...
export function CardFrontRestaurant({ card, theme, language, isSaving, onSaveContact }: CardFrontRestaurantProps) {
  ...
  const divider = dark ? 'border-zinc-800/60' : 'border-zinc-200/80';
  ...
  return (
    <div className={`w-full bg-linear-to-br ${cardBg} rounded-3xl border backdrop-blur-2xl ${cardShadow} flex flex-col transition-colors duration-300 relative`}>
      ...
      {/* ── Partie haute — identique aux autres templates ── */}
        ...
        <div className="px-4 py-4 flex flex-col gap-4">
          ...
          {Object.entries(grouped).map(([category, items]) => (
            <div key={category} className="flex flex-col gap-1.5">
              <p className={`text-[11px] font-semibold ${sectionTitle} tracking-wide uppercase`}>{category}</p>
              {items.map((item) => (
                <div
                  key={item.id}
                  className={`flex items-center justify-between px-3 py-2 rounded-xl border ${itemBg}`}
                >
