        ...
        >
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm mt-2">
              Bienvenue <span className="text-white font-medium">{name}</span>.
              <br />Un email de vérification a été envoyé à{' '}
              <span className="text-white font-medium">{email}</span>.
            </p>
          </div>
          <Link href="/login" className="text-sm text-orange-500 hover:text-orange-400 transition-colors font-medium">
            Se connecter
          </Link>
        </motion.div>
      </div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <div className="flex gap-1 flex-1">
                    {[1, 2, 3].map((lvl) => (
                      <div
                        key={lvl}
                        className={`h-1 flex-1 rounded-full transition-all duration-300 ${strength >= lvl ? strengthColor[strength] : 'bg-zinc-700'}`}
                      />
                    ))}
                  </div>
                  <span className={`text-xs font-medium ${strength === 1 ? 'text-rose-400' : strength === 2 ? 'text-orange-400' : 'text-emerald-400'}`}>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  required
                  className={`w-full pl-9 pr-10 py-2.5 text-sm bg-zinc-800/60 border rounded-xl text-zinc-100 placeholder:text-zinc-500 outline-none transition-all ${
                    confirmPassword.length > 0 && confirmPassword !== password
                      ? 'border-rose-500/50 focus:ring-1 focus:ring-rose-500/20'
                      : 'border-zinc-700/40 focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/20'
                  }`}
                />
                <button
                  type="button"
